...
export default class GameScene extends Phaser.Scene {
	constructor() {
		...
		this.gameConfig = {
			...
			burntTime: 10000, // 10秒后烤糊（烹饪完成后还有7秒缓冲时间）
	...
	updateWashStationsFromServer(serverWashStations) {
		Object.keys(serverWashStations).forEach((washStationId) => {
			...
			if (localWashStation) {
				...
				// 更新洗碗槽状态
				localWashStation.setData('isWashing', serverWashStationData.isWashing);
				localWashStation.setData(
					'cleanPlate',
					serverWashStationData.cleanPlate
				);
	...
	setupParticleEffects() {
		// 创建粒子效果
		this.cookingParticles = this.add.particles(0, 0, 'progress_bar', {
	...
	handleWashStationInteraction(player, washStation) {
		...
		if (Phaser.Input.Keyboard.JustDown(this.spaceKey)) {
			...
			if (cleanPlate && !this.playerHolding) {
				...
				this.showMessage('取回了干净的盘子', 0x2ed573);
		...
		if (Phaser.Input.Keyboard.JustDown(this.eKey)) {
			...
			if (cleanPlate) {
				this.showMessage('请先用空格键取回干净的盘子', 0xffa502);
				return;
			}
	...
	startAutoCooking(station, stationType, resultType, processTime) {
		...
		// 着火计时器（超时会着火）
		const fireTimer = this.time.delayedCall(this.gameConfig.burntTime, () => {
			this.startFire(station, stationType);
		});
	...
	placePreparedPlateOnGround(x, y) {
		...
		if (this.isValidPlacementPosition(x, y)) {
			// 在地面创建装好的盘子，保留其内容
			const plate = this.plates.create(x, y, 'plate'); // 使用普通盘子纹理
			plate.setData('contents', [...this.playerHolding.contents]); // 保存盘子内容
	...
	completeAutoCooking(station, stationType, resultType) {
		...
		this.showMessage('烹饪完成！按空格键取回食材', 0xffd700);
	...
	startFire(station, stationType) {
		...
		station.setTexture('cooking_station_fire'); // 着火纹理
